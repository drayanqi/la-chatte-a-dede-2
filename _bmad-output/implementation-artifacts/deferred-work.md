...
- **Tactic color-default literals duplicated** — `#ff6b1a`/`#1a8cff` are hardcoded in `Tactic::$attributes`, `store()`'s effective-pair defaults and the create() fallbacks (TacticController.php:53-54,76-77 vs Tactic.php:57-58); a column-default drift would desync the distinctness check from reality — extract `Tactic::COLOR_PRIMARY_DEFAULT`/`COLOR_SECONDARY_DEFAULT` constants in a hygiene pass. [surfaced by the team-kit-colors review, 2026-09-24]
- **Static CSS-var fallbacks in ReplayDrawer sparkline/bars** — `var(--home-accent, #e4573f)` / `var(--away-accent, #3d8fd1)` fallbacks (ReplayDrawer.tsx:530,533,687,691,751,759) are unreachable (the vars are always set inline from resolved kits) and the fallback hexes are the old base palette — drop the fallbacks or move them to design tokens in a theming pass. [surfaced by the team-kit-colors review, 2026-09-24]
- **Dead `LOG_TEAM_COLORS` in replayLogs.ts** — static base-palette map with zero consumers since the drawer moved to resolved kits (team-kit-colors); remove it (or fold into the theming pass) alongside `LOG_LEVEL_COLORS`. [surfaced by the team-kit-colors review, 2026-09-24]
