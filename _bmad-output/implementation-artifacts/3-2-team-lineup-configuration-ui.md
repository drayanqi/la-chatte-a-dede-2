...
- **Default formation** (percent units, matches Story 3.6 bot geometry; API accepts x 0-100 for legacy rows, y 0-50): slot 1 GK (8, 25), slot 2 DEF (25, 15), slot 3 DEF (25, 35), slot 4 ATK (40, 15), slot 5 ATK (40, 35). All `scriptId: null` on creation. Tactic positions are kickoff positions: home players are constrained to the left half (x ≤ 50) because home defends the left goal and attacks toward x=100.
- **Coordinate bridge:** the engine renders y in 0-100 (full field height) while the API stores y in 0-50 (half-field units) — `src/lib/tacticBridge.ts` scales y by ×2 on the way in and ÷2 on the way out. The bridge also normalizes x into [0, 50] (mirror right-half x across the halfway line, then clamp): legacy x > 50 values are healed on load and do not round-trip identity. Engine player ids remain stable (`home-{slot-1}`) so save round-trips cleanly for in-half positions.
